...
class DatabaseHelper private constructor(context: Context, password: CharArray) :
    ...
    fun getAllEntries(): List<Entry> {
        ...
        val cursor: Cursor = db.query(TABLE_ENTRIES, null, null, null, null, null, "$COL_DATE DESC")
        cursor.use {
            while (it.moveToNext()) {
                list += Entry(
    ...
    fun exportRawFile(context: Context): File =
        context.getDatabasePath(DB_NAME)
    ...
    companion object {
        ...
        fun getInstance(context: Context, password: CharArray): DatabaseHelper {
            return instance ?: synchronized(this) {
                instance ?: DatabaseHelper(context.applicationContext, password).also {
                    instance = it
                }
            }
        }
